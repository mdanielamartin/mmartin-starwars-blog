        ...
        <div className="row w-100">
            <ul className="nav nav-tabs background-card rounded-top-4">
                <li className="nav-item">
                    <button className={`buttons background-card regular p-3 fs-5 border-0 rounded-4 ${display === 'General' ? 'bg-light' : ''}`} name="General" onClick={(e) => setDisplay(e.target.name)} aria-current="page">General Information</button>
                </li>
                <li className="nav-item">
                    <button className={`buttons background-card regular p-3 fs-5 border-0 rounded-4 ${display === 'Specs' ? 'bg-light' : ''}`} name='Specs' onClick={(e) => setDisplay(e.target.name)} aria-current="page">Specifications</button>
                </li>
                <li className="nav-item">
                    <button className={`buttons background-card regular p-3 fs-5 border-0 rounded-4 ${display === 'Capacity' ? 'bg-light' : ''}`} name='Capacity' onClick={(e) => setDisplay(e.target.name)} aria-current="page">Capacity & Payload</button>
                </li>
                <li className="nav-item">
                    <button className={`buttons background-card regular p-3 fs-5 border-0 rounded-4 ${display === 'Financials' ? 'bg-light' : ''}`} name='Financials' onClick={(e) => setDisplay(e.target.name)} aria-current="page">Financials & Operations</button>
                </li>
            </ul>
